...
class Pawn{ //extends {
	...
	public Pawn(parent, Vector pos, String color, boolean playable){
		 ...
		 *parent est la variable qui correspond à l'objet de javafx qui contient le pion
		 *pos correspond à la position qui est donnée au pion lors de sa construction
		...
		if (this.color.equals(white))
			//mettre l'image correspondant à un pion blanc
		else
			//mettre l'image correspondant à un pion noir
	}

	public void destroyPossibilities(){
		//on détruit les objets mis dans le tableau des possibilités

		for (int i = 0; i < possibilities.length(); i++)
			//possibilities[i].destroy() //.hide()
	}

	public void Clicked(){ //événement /!\
		click_num++;
		click_num %= 2;
		if (click_num > 1e-100){
			//si c'est la première fois qu'on appuye sur le pion
			//alors on montre les possibilités de mouvement
			checkCase();
			checkWall();
			ShowPossibilities();
		}
		else{
			//c'est que c'est la deuxième fois qu'on appuie sur le pion
			//donc on cache les possibilités
			destroyPossibilities();
		}
	}

	public void ShowPossibilities(){
		//on montre les possibilités sur le plateau, grâce à un tableau de possibilités
	...
	public void checkWall(){
		...
		for (int i = 0; i < cases_possibilities.length(); i++){
			...
			if (mur_case.equals("w"))
				...
				if (mur_case.equals("w")){
					Vector [] sub_list;
					//s'il y a un mur derrière le pion, mvmts en L
	...
	public void checkCase(){
		//vérifie qu'il y ait bien une case pour les possibles mouvements
		...
		if ((pos.x-down.x*2) > -1 and (pos.y-down.y*2) > -1){
			//voir si la case au-dessus existe
			cases[0] = true;
			num_of_possib++;
		}

		if ((pos.x+down.x*2) < 17 and (pos.y+down.y*2) < 17){
			//voir si la case en-dessous existe
			cases[1] = true;
			num_of_possib++;
		}

		if ((pos.x-right.x*2) > -1 and (pos.y-right.y*2) > -1){
			//voir si la case à gauche existe
			cases[2] = true;
			num_of_possib++;
		}

		if ((pos.x+right.x*2) < 17 and (pos.y+right.y*2) < 17){
			//voir si la case à droite existe
			cases[3] = true;
			num_of_possib++;
		}
	...
	public void move(Vector newpos){
		//bouge le pion à la nouvelle position newpos passée en argument
		//vérifie si le joueur a gagné la partie grâce à ce mouvement
		//une fois fini, on passe la main à l'autre joueur
	...
	public void win(){
		//vérifie si la position actuelle est une case gagnante
